...
#define DEFAULT_NODE_BASE_PORT 9001
...
static int send_u8(int fd, uint8_t v) {
    return send_all(fd, &v, 1);
}

static int recv_u8(int fd, uint8_t *v) {
...
static int send_string(int fd, const char *s, uint32_t len) {
    if (send_u32(fd, len) < 0) return -1;
    if (len > 0 && send_all(fd, s, (int)len) < 0) return -1;
    return 0;
}

static char *recv_string(int fd, uint32_t *out_len) {
    uint32_t len = 0;
